...
export class ShopByCategory extends LitElement {
  ...
  private _renderPromotion(
    item: BannerItem
  ): TemplateResult | string {

    if (!item.banner_promotion_txt) return "";
  ...
  render() {
    ...
    const banners =
      cfg?.B_banner_sec_collection ?? [];

    const sectionId =
      `B_shop_by_category-${this.position}`;
